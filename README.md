# Tractorbeam Node API Library

[![NPM version](https://img.shields.io/npm/v/tractorbeam.svg)](https://npmjs.org/package/tractorbeam) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/tractorbeam)

This library provides convenient access to the Tractorbeam REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const params: Tractorbeam.GraphCreateParams = {};
  const graph: unknown = await client.graphs.create(params);
}
...
```ts
async function main() {
  const graph = await client.graphs.create({}).catch(async (err) => {
...
const response = await client.graphs.create({}).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: graph, response: raw } = await client.graphs.create({}).withResponse();
...
add the following import before your first import `from "Tractorbeam"`:
